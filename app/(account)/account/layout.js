...
export default function Layout({ children }) {
  ...
  const generateBreadcrumbs = () => {
    ...
    pathSegments.forEach((segment, index) => {
      ...
      if (segment.match(/^[0-9a-fA-F]{24}$/) && pathSegments[index - 1] === "orders") {
        breadcrumbs.push({
          href: "#",
          label: "Order Details",
          active: true,
      ...
      breadcrumbs.push({
        href: isLast ? "#" : href,
        label: link ? link.label : segment.charAt(0).toUpperCase() + segment.slice(1),
        active: isLast,
      })
    })
  ...
  return (
    <div className="bg-gray-50">
      <div className="md:py-12 relative mx-auto min-h-screen max-w-4xl px-7 py-8 sm:px-8 sm:py-14 md:px-10">
