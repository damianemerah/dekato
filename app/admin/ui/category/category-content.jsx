...
const Categories = ({ searchParams, data }) => {
  ...
  const cancelChanges = (key) => {
    setPinOrders((prev) => ({
      ...prev,
      [key]: categories?.data.find((item) => item.id === key)
        ? {
            pinOrder: categories?.data.find((item) => item.id === key).pinOrder,
            isChecked: !!categories?.data.find((item) => item.id === key)
              .pinned,
          }
        : undefined,
    }));
  ...
  const dataSource = Array.isArray(categories?.data)
