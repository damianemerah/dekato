    ...
    <>
      <div className="relative z-[55] hidden lg:block">
        <form onSubmit={(e) => handleSearchProduct(e)}>
          ...
          >
            <SearchOutlined className="h-5 w-5 text-primary" />
          </button>
        </form>
      ...
      <div className="lg:hidden">
        ...
        >
          <SearchOutlined className="h-5 w-5 text-white" />
        </button>
      </div>
      {showMobileSearch && (
        <div className="absolute left-0 right-0 top-16 z-[60] !m-0 w-full bg-white py-2 lg:hidden">
          ...
          >
            <SearchOutlined className="absolute left-4 top-1/2 z-10 -translate-y-1/2 transform !text-primary" />
            <input
              ref={mobileSearchRef}
              onFocus={() => setActiveDropdown(true)}
              type="text"
              placeholder="Search..."
              className="h-10 w-full bg-white px-12 py-2 text-primary outline-none placeholder:text-sm"
              onChange={(e) => setSearchString(e.target.value)}
            />
            <button
              type="button"
            ...
            >
              <CloseOutlined className="h-5 w-5 !text-primary" />
            </button>
          </form>
          {pSearchList?.products && activeDropdown && (
            <ul
